...
This module intentionally uses only the Python standard library. It installs a
self-contained Agentit runtime, a private Python environment for runtime
Python dependencies, a small CLI shim, and provider discovery surfaces. Human
users are not expected to drive these commands manually; coding agents can
inspect the plan and apply it on their behalf.

Semantic task decisions never happen here. This is mechanical packaging only.
"""
...
def _destination(home: Path, relative: str | Path) -> Path:
    ...
    dest = (home / rel).absolute()
    try:
        dest.relative_to(home)
    except ValueError as exc:  # pragma: no cover - defensive after _assert_relative
        raise BootstrapError(f"destination escapes home: {dest}") from exc
    _assert_no_symlink_components(dest, stop=home)
    return dest
...
def _excluded(relative: Path, manifest: dict[str, Any]) -> bool:
    names = set(str(x) for x in manifest.get("runtime_exclude_names") or [])
    prefixes = tuple(str(x) for x in manifest.get("runtime_exclude_prefixes") or [])
...
def _runtime_specs(
    *, home: Path, source_root: Path, manifest: dict[str, Any]
) -> list[CopySpec]:
...
def _provider_specs(
    *,
    ...
    specs: list[CopySpec] = []
    provider_catalog = manifest.get("providers") or {}
    core_skills = [str(x) for x in manifest.get("core_skills") or []]
    ...
    for provider in providers:
        ...
        if provider == "claude" and config.get("agents_source"):
            src = _source(source_root, str(config["agents_source"]))
            dest = _destination(home, str(config["agents_root"]))
            _append_tree_specs(
                specs,
                source_root=source_root,
                source_base=src,
                destination_base=dest,
                category="provider:claude:agent",
            )

        if provider == "codex" and config.get("codex_agents_source"):
            src_root = _source(source_root, str(config["codex_agents_source"]))
            dest_root = _destination(home, str(config["agents_root"]))
            for filename in manifest.get("codex_agent_profiles") or []:
                filename = str(filename)
                src = _source(source_root, Path(config["codex_agents_source"]) / filename)
                specs.append(
                    CopySpec(
                        source=src,
    ...
    if with_settings:
        item = optionals.get("claude_settings") or {}
        specs.append(
            _optional_spec(home=home, source_root=source_root, item=item, category="optional:claude-settings")
        )
    if with_local_settings:
        item = optionals.get("claude_local_settings") or {}
        specs.append(
            _optional_spec(home=home, source_root=source_root, item=item, category="optional:claude-local-settings")
        )
    if with_hook:
        item = optionals.get("claude_hook") or {}
        specs.append(
            _optional_spec(home=home, source_root=source_root, item=item, category="optional:claude-hook")
        )
    return specs


def _optional_spec(
    *, home: Path, source_root: Path, item: dict[str, Any], category: str
) -> CopySpec:
    ...
    mode = int(str(raw_mode), 8) if raw_mode else stat.S_IMODE(src.stat().st_mode)
    return CopySpec(
        source=src,
...
def _cli_spec(home: Path) -> CopySpec:
    ...
    script = (
        ...
        f"AGENTIT = {str(runtime_agentit)!r}\n"
        ...
        if spec.destination.exists():
            if not spec.destination.is_file() or spec.destination.is_symlink():
                raise BootstrapError(
                    f"existing destination is not a regular file: {spec.destination}"
                )
            current_hash = _sha256_file(spec.destination)
            action = "keep" if current_hash == desired_hash else "replace-with-backup"
        else:
            action = "install"
        operations.append(
            {
...
def _install_dependencies(
    *, home: Path, dependencies: list[str], skip_dependencies: bool
) -> Path:
    ...
    if dependencies and not skip_dependencies:
        ...
        if completed.returncode != 0:
            raise BootstrapError(
                "failed to install Agentit Python dependencies:\n" + completed.stdout[-4000:]
            )
    return python
...
def apply_install_plan(
    ...
    *,
    ...
    _install_dependencies(
        home=home,
        dependencies=[str(x) for x in plan.get("python_dependencies") or []],
    ...
    for spec in specs:
        ...
        if destination.exists():
            ...
            _assert_no_symlink_components(backup_path, stop=backup_root)
            _atomic_write(
                backup_path,
    ...
    return {
        ...
        "path_note": (
            "The coding agent may call cli_path directly. ~/.local/bin need not be on the human user's PATH."
        ),
    }
...
def rollback_plan(manifest_path: Path) -> dict[str, Any]:
    ...
    for record in reversed(receipt.get("records") or []):
        ...
        current_hash = _sha256_file(destination)
        if current_hash != record.get("installed_sha256"):
            raise BootstrapError(
                f"refusing rollback because destination changed after install: {destination}"
            )
        if record.get("before_state") == "present":
            backup = Path(str(record.get("backup_path")))
            if not backup.is_file() or backup.is_symlink():
                raise BootstrapError(f"rollback backup missing/unsafe: {backup}")
            if _sha256_file(backup) != record.get("backup_sha256"):
                raise BootstrapError(f"rollback backup hash mismatch: {backup}")
            action = "restore"
...
def apply_rollback(manifest_path: Path) -> dict[str, Any]:
    ...
    record_by_destination = {
        str(record["destination"]): record for record in receipt.get("records") or []
    }
    changed = 0
    for operation in plan["operations"]:
        ...
        if operation["action"] == "restore":
            ...
            mode = int(record["original_mode"])
            _atomic_write(destination, backup.read_bytes(), mode=mode, home=home)
    ...
    return {
        ...
        "note": (
            "The private ~/.agentit/venv and now-empty managed directories are intentionally not recursively deleted."
        ),
    }


def _public_plan(plan: dict[str, Any]) -> dict[str, Any]:
    return {key: value for key, value in plan.items() if key != "_specs"}


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="agentit bootstrap",
        description="Plan/apply Agentit's portable agent-facing bootstrap.",
    )
    parser.add_argument("--provider", choices=("all", "claude", "codex", "antigravity"), default="all")
    parser.add_argument("--home", type=Path, default=Path.home())
    parser.add_argument("--source-root", type=Path, default=SOURCE_ROOT)
    parser.add_argument("--apply", action="store_true")
    parser.add_argument("--backup-dir", type=Path)
    parser.add_argument("--with-settings", action="store_true", help="Opt-in Claude settings.json replacement; not part of the normal launch path.")
    parser.add_argument("--with-local-settings", action="store_true", help="Opt-in copy from the tracked example into machine-local Claude settings.")
    parser.add_argument("--with-hook", action="store_true")
    parser.add_argument("--skip-dependencies", action="store_true", help="Testing/offline only: do not pip-install runtime Python dependencies into Agentit's private venv.")
