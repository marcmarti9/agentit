...
if [[ -d "$auth_root" ]]; then
  ...
  symlink_list_file="$(mktemp /tmp/agentit-cursor-symlinks.XXXXXX)"
  ...
  auth_list_file="$(mktemp /tmp/agentit-cursor-find.XXXXXX)"
...
if [[ "$MODE" == "apply" ]]; then
  if [[ -z "$BACKUP_ROOT" ]]; then
    BACKUP_ROOT="$USER_HOME/backups/agentit-local-hardening-$(date +%Y%m%d-%H%M%S)"
  fi
