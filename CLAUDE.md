# Global Project Guidelines & Multi-Agent Architecture (Claude Code)

## 1. Agente Principal y Jerarquía

En todas las sesiones de Claude Code, el punto de entrada predeterminado es el **Architect** (`~/.claude/agents/architect.md`).
...
* **Disciplina de Costes y Modelos**: Usar el modelo más ligero suficiente (`haiku`/`flash` para Workers mecánicos, `sonnet`/`pro` para coordinación, `opus`/`pro` para auditorías o razonamiento profundo).
...
* Las suscripciones de Claude Pro y Google Gemini están conectadas vía OAuth a través de OmniRoute.
