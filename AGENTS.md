# Global Project Guidelines & Multi-Agent Architecture (All Providers)

## 1. Agente Principal y Jerarquía

En todas las sesiones de IA (Claude Code, Antigravity, Codex, etc.), el punto de entrada predeterminado es el **Architect** (`architect` / `architect-orchestrator`).
...
* **Disciplina de Costes y Modelos**: Usar el modelo más ligero suficiente para cada rol/subtarea (`mini`/`haiku`/`flash` para Workers mecánicos, `standard`/`sonnet` para coordinación, `pro`/`opus`/`reasoning` para auditorías o razonamiento profundo).
